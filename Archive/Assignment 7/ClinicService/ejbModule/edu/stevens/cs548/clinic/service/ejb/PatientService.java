...
public class PatientService implements IPatientServiceLocal, IPatientServiceRemote {
	...
	@Override
		public void addDrugTreatment(long id, long providerId, String diagnosis, String drug,
			float dosage) throws PatientNotFoundExn {
		try {
			Patient patient = patientDAO.getPatientByDbId(id);
			patient.addDrugTreatment(diagnosis, drug, dosage, providerId);
		} catch(PatientExn e){
	...
	public void addSurgery(long id, String diagnosis, Date date, long providerId) throws PatientNotFoundExn {
		try {
			Patient patient = patientDAO.getPatientByDbId(id);
			patient.addSurgery(diagnosis, date, providerId);
		} catch(PatientExn e){
			throw new PatientNotFoundExn(e.toString());
		}		
	}
	...
	public void addRadiology(long id, String diagnosis, List<Date> dates, long providerId) throws PatientNotFoundExn {
		try {
			Patient patient = patientDAO.getPatientByDbId(id);
			patient.addRadiology(diagnosis, dates, providerId);
		} catch(PatientExn e){
			throw new PatientNotFoundExn(e.toString());
		}
	}
